...
1. **Code shipping through pipelines with no SAST or SCA checks**, allowing vulnerable or malicious dependencies to reach production undetected.
...
The study is a structured control-mapping exercise. I identified seven security findings in a real, publicly available federal agency GitHub repository (everything reproducible by any reviewer with a free GitHub account) and applied a coding rubric to rate how well SP 800-204D addresses each one.
...
The hypothesis is that SP 800-204D will cover several of the findings at the intent level, but a non-zero gap surface will remain, particularly for findings that touch FedRAMP boundary controls and application-configuration concerns that fall outside the document's explicit scope.
...
The goal of this whitepaper is modest and specific: demonstrate a repeatable mapping protocol, surface where the guidance lands and where it falls short, and give practitioners something they can cite when making the case for pipeline security investment to leadership.
